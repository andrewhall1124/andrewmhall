<script lang="ts">
	import { getPoints, getWinner, getValidMoves, getComputerMove, flipChips } from '$lib/othello';

	function createSquares(m: number, n: number, v: number | null): (number | null)[][] {
	...
	function move(coordinates: number[]): void {
		...
		squares[i][j] = turn;
		squares = flipChips(squares, turn, [i, j]);
		changeTurn();
	}
	...
	async function makeComputerMove(player: string): Promise<void> {
		return new Promise((resolve) => {
			// Use setTimeout to allow UI to update first
			setTimeout(() => {
				const computerMove = getComputerMove(squares, turn, player);
				if (computerMove) {
					move(computerMove);
				}
				resolve();
	...
	$effect(() => {
		if (turn == 0 && blackPlayer != 'human' && winner == null) {
			makeComputerMove(blackPlayer);
		}
	});

	$effect(() => {
		if (turn == 1 && whitePlayer != 'human' && winner == null) {
			makeComputerMove(whitePlayer);
		}
	});
